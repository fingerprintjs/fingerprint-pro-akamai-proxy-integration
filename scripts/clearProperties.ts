...
const checkDNSRecordExists = async (domain: string) => {
  const response = await fetch(
    `https://api.cloudflare.com/client/v4/zones/${process.env.CF_ZONE_ID}/dns_records?name=${domain}`,
    {
      headers: {
    ...
    if (!dnsRecordExists) {
      ...
      for (const activation of versionsToDeactivate) {
        try {
          ...
          console.log(
            'SUCCESS:DEACTIVATE',
            `property(${property.propertyId}) version(${activation.version}) network(${activation.network})`,
          )
        } catch (e: any) {
